package com.project.nicki.displaystabilizer.dataprovider.orientationProvider;
...
public class ImprovedOrientationSensor1Provider extends OrientationProvider {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR) {
            ...
            // after computing it from the gyro sample buffer.
            if (timestamp != 0) {
        ...
        com.project.nicki.displaystabilizer.dataprovider.representation.Quaternion quaternion = new com.project.nicki.displaystabilizer.dataprovider.representation.Quaternion();
        currentOrientationProvider.getQuaternion(quaternion);

        init.initglobalvariable.sQuaternionVal.add(event.timestamp,new float[]{
                currentOrientationProvider.currentOrientationQuaternion.getW(),
                currentOrientationProvider.currentOrientationQuaternion.getX(),
                currentOrientationProvider.currentOrientationQuaternion.getY(),
                currentOrientationProvider.currentOrientationQuaternion.getZ()
        });

        init.initglobalvariable.sDevice2World  = new org.apache.commons.math3.complex.Quaternion(
                init.initglobalvariable.sQuaternionVal.getLatestData().getValues()[0],
                init.initglobalvariable.sQuaternionVal.getLatestData().getValues()[1],
                init.initglobalvariable.sQuaternionVal.getLatestData().getValues()[2],
                init.initglobalvariable.sQuaternionVal.getLatestData().getValues()[3]
