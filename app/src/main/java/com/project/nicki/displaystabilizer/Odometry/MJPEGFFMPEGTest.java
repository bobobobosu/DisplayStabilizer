        ...
        if (recording) {
            ...
            if (p.getPreviewFormat() == ImageFormat.NV21) {
                ...
                try {
					...
					/*
                    //new
                    ImageType<ImageUInt8> type = new ImageType<ImageUInt8>(ImageType.Family.MULTI_SPECTRAL, ImageDataType.U8,8);
                    ImageUInt8 toprocess = new ImageUInt8(p.getPreviewSize().width,p.getPreviewSize().height);
                    ConvertNV21.nv21ToBoof(b, p.getPreviewSize().width, p.getPreviewSize().height ,toprocess);
                    InputStream  imgInputStreaam = new ByteArrayInputStream(b);
                    ImageStreamSequence<ImageUInt8> mImageStreamSequence = new ImageStreamSequence<ImageUInt8>(imgInputStreaam,true,type);
				*/
                    //new

                    ImageType<ImageUInt8> type = new ImageType<ImageUInt8>(ImageType.Family.MULTI_SPECTRAL, ImageDataType.U8, 8);
                    ImageUInt8 img = new ImageUInt8(p.getPreviewSize().width, p.getPreviewSize().height);
                    ConvertNV21.nv21ToBoof(b, p.getPreviewSize().width, p.getPreviewSize().height, img);
                    InputStream imgInputStreaam = new ByteArrayInputStream(b);
                    //ImageStreamSequence<ImageUInt8> mImageStreamSequence = new ImageStreamSequence<ImageUInt8>(imgInputStreaam,true,type);


                    Bitmap mbitmap = null;
                    Bitmap.createBitmap(p.getPreviewSize().width, p.getPreviewSize().height, Bitmap.Config.ARGB_8888);
                    VisualizeImageData.binaryToBitmap(img, false, mbitmap, b);
                    ImageUInt8 image = ConvertBitmap.bitmapToGray(mbitmap, (ImageUInt8) null, null);

                    ImageStreamSequence<ImageUInt8> mImageStreamSequence = new ImageStreamSequence<ImageUInt8>(imgInputStreaam, true, type);

                    Log.d("aaaaaaaaa", "aaaaaaaaaaaaaaaaaaaaaa");
                    //ConvertNV21.nv21ToGray(b, p.getPreviewSize().width, p.getPreviewSize().height ,gray1);
