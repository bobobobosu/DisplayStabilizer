...
public class LevenbergMarquardt {
    ...
    private boolean adjustParam(DenseMatrix64F X, DenseMatrix64F Y,
                                double prevCost, ArrayList<proAcceGyroCali.sensordata> data) {
     ...
     */
    private void computeDandH(DenseMatrix64F param, DenseMatrix64F x, DenseMatrix64F y, ArrayList<proAcceGyroCali.sensordata> data) {
        func.compute(param, x, tempDH, data);
        subtractEquals(tempDH, y);

        computeNumericalJacobian(param, x, jacobian, data);
     ...
     */
    private double cost(DenseMatrix64F param, DenseMatrix64F X, DenseMatrix64F Y, ArrayList<proAcceGyroCali.sensordata> data) {
        func.compute(param, X, temp0, data);
    ...
    protected void computeNumericalJacobian(DenseMatrix64F param,
                                            DenseMatrix64F pt,
                                            DenseMatrix64F deriv, ArrayList<proAcceGyroCali.sensordata> data) {
        double invDelta = 1.0 / DELTA;

        func.compute(param, pt, temp0, data);
        ...
        for (int i = 0; i < param.numRows; i++) {
            param.data[i] += DELTA;
            func.compute(param, pt, temp1, data);
